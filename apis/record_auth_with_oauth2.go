...
func oldCanAssignUsername(txApp core.App, collection *core.Collection, username string) bool {
	...
	_, hasUniqueue := dbutils.FindSingleColumnUniqueIndex(collection.Indexes, field.GetName())
	if hasUniqueue {
		...
		expr := dbx.NewExp("LOWER([["+colName+"]]) = LOWER({:username})", dbx.Params{"username": username})
