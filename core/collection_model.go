...
func onCollectionDeleteExecute(e *CollectionEvent) error {
	...
	if !e.Collection.disableIntegrityChecks {
		...
		// note: the select is outside of the transaction to prevent deadlocks when mixing read&write in a single transaction
...
func onCollectionSaveExecute(e *CollectionEvent) error {
	...
	if !e.Collection.IsNew() {
		...
		// warn when an identity field leaves PasswordAuth.IdentityFields while
		// its functional unique index stays behind (D-2 "keep" lifecycle): the
		// index keeps enforcing case-insensitive uniqueness after the field is
		// no longer used for login, which can confuse admins with duplicate
		// errors. No cleanup happens automatically - this is a user decision.
		warnKeptIdentityFieldIndexes(e.App, oldCollection, e.Collection)
	}
...
func (c *Collection) initEmailField() {
	...
	c.ensureFunctionalUniqueIndex(FieldNameEmail)
}

// functionalUniqueIndexSQL returns a functional partial unique index
// expression for the specified identity field of the form:
//
//	CREATE UNIQUE INDEX "idx_<field>_<id>" ON "<collection>" (LOWER("<field>")) WHERE "<field>" <> ''
func (c *Collection) functionalUniqueIndexSQL(fieldName string) string {
	return fmt.Sprintf(
		`CREATE UNIQUE INDEX "%s" ON "%s" (LOWER("%s")) WHERE "%s" <> ''`,
		c.fieldIndexName(fieldName),
		c.Name,
		fieldName,
		fieldName,
	)
...
// ensureFunctionalUniqueIndex ensures that the collection has a functional
// partial unique (case-insensitive) index for the provided identity field,
// appending one when no single-column unique index exists on the field yet.
//
// The function is intentionally append-only: converting an existing plain
// (case-sensitive) unique index to the functional form is the job of the
// identity-index migration, and rejecting a non-functional index on an active
// identity field is the job of the collection validator. This keeps the
// onSave normalization side-effect-free (it never silently rewrites a stored
// index definition).
//
// Only call this for text-compatible identity fields: building LOWER() over a
// non-text column (eg. number/json) would fail on the database side.
func (c *Collection) ensureFunctionalUniqueIndex(fieldName string) {
	// a single-column unique index (plain or functional) already covers the
	// identity field -> nothing to do
	if _, ok := dbutils.FindSingleColumnUniqueIndex(c.Indexes, fieldName); ok {
		return
	}

	c.Indexes = append(c.Indexes, c.functionalUniqueIndexSQL(fieldName))
}

// initIdentityFieldIndexes ensures that every password-auth identity field
// (eg. username, custom identity fields) has a functional partial unique index
// so the LOWER(field) = LOWER(?) auth lookups are index-served and
// case-variant duplicate identities are rejected.
//
// Email is intentionally skipped here: it is a system field fully managed by
// initEmailField (and protected from manual change by the system index
// validator), so a second generator would only duplicate the work.
func (c *Collection) initIdentityFieldIndexes() {
	if !c.IsAuth() {
		return
	}

	for _, fieldName := range list.ToUniqueStringSlice(c.PasswordAuth.IdentityFields) {
		if fieldName == "" || strings.EqualFold(fieldName, FieldNameEmail) {
			continue
		}

		field := c.Fields.GetByName(fieldName)
		if field == nil {
			continue // let the collection validator handle invalid identity fields
		}

		// only text-compatible fields can be indexed via LOWER(...)
		if field.Type() != FieldTypeText && field.Type() != FieldTypeEmail {
			continue
		}

		c.ensureFunctionalUniqueIndex(fieldName)
	}
}

// removedIdentityFields returns the identity fields that were present in the
// old collection's PasswordAuth.IdentityFields but are no longer in the new
// collection's.
func removedIdentityFields(oldCollection, newCollection *Collection) []string {
	removed := []string{}

	oldIdentity := list.ToUniqueStringSlice(oldCollection.PasswordAuth.IdentityFields)
	newIdentity := list.ToUniqueStringSlice(newCollection.PasswordAuth.IdentityFields)

	for _, fieldName := range oldIdentity {
		if fieldName == "" {
			continue
		}
		if !list.ExistInSlice(fieldName, newIdentity) {
...
// warnKeptIdentityFieldIndexes logs a warning when an identity field has left
// PasswordAuth.IdentityFields but its unique index remains on the collection.
//
// The functional unique index is intentionally kept (D-2 "keep" lifecycle) and
// continues to enforce case-insensitive uniqueness, so a removed identity field
// can still cause duplicate errors. This warning is the user-facing signal for
// that behavior so they can decide whether to drop the now-orphaned index
// manually.
func warnKeptIdentityFieldIndexes(app App, oldCollection, newCollection *Collection) {
	if oldCollection == nil || newCollection == nil || !newCollection.IsAuth() {
		return
	}

	for _, fieldName := range removedIdentityFields(oldCollection, newCollection) {
		// only warn if the field's unique index actually still exists
		idx, ok := dbutils.FindSingleColumnUniqueIndex(newCollection.Indexes, fieldName)
		if !ok {
			continue
		}

		app.Logger().Warn(
			`The identity field "`+fieldName+`" is no longer used for login but its unique index `+idx.IndexName+` is kept. Case-insensitive uniqueness is still enforced; remove the index manually if this is no longer desired.`,
