...
type DBConnectFunc func(config DBConfig) (*dbx.DB, error)
...
// BaseApp implements core.App and defines the base PG-BASE app structure.
type BaseApp struct {
...
func (app *BaseApp) initLogger() error {
	...
	app.OnSettingsReload().Bind(&hook.Handler[*SettingsReloadEvent]{
		...
		Func: func(e *SettingsReloadEvent) error {
			...
			_, err = e.App.AuxDB().Delete((&Log{}).TableName(), expr).Execute()
			if err != nil {
