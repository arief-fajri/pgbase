...
func (r *runner) processRequestBodyEachModifier(bodyField Field) (*search.ResolverResult, error) {
	...
	jeTable := fmt.Sprintf("jsonb_array_elements({:%s}::jsonb) AS je(value)", placeholder)
	...
	if r.withMultiMatch {
		...
		jeTable2 := fmt.Sprintf("jsonb_array_elements({:%s}::jsonb) AS je(value)", placeholder2)
	...
	for i, prop := range r.activeProps {
		...
		if field == nil {
			...
			if backRelField.CollectionId != collection.Id {
				// https://github.com/arief-fajri/pgbase/discussions/6590#discussioncomment-12496581
				if r.nullifyMisingField {
					return &search.ResolverResult{Identifier: "NULL"}, nil
				}
