...
func (app *BaseApp) CreateBackup(ctx context.Context, name string) error {
	...
	return app.OnBackupCreate().Trigger(event, func(e *BackupEvent) error {
		...
		// captures the whole database (not just the storage filesystem). The dump
		// format is selected by the Backups.Format setting:
		...
		createErr := e.App.RunInTransaction(func(txApp App) error {
			// export within the write-blocking transaction so the database dump and
			...
			var exportErr error
			if format == BackupFormatSQLite {
				exportErr = bapp.ExportToSQLiteFile(e.Context, dbDumpPath)
			} else {
				exportErr = bapp.ExportToPGDump(e.Context, dbDumpPath)
			}
			if exportErr != nil {
				return fmt.Errorf("failed to export the database snapshot: %w", exportErr)
			}
