...
> Audience: All (Contributor | Operator | App Builder) · Status: living document · Last verified: v0.5.2 (`923e860`)
...
- **Production docs** — `../PRODUCTION.md` runbook, `docker-compose.prod.yml`
...
**Deliverable:** a "Running multiple instances" section in `../PRODUCTION.md` with a
...
| **PERF-I08** | User index-add on populated collections uses blocking `CREATE INDEX` inside a tx | Out-of-tx `CREATE INDEX CONCURRENTLY` path (documented escape hatch exists in `../DEV.md` §5b) | P2 / L |
...
should include a multi-instance test plan. See `../CONTRIBUTING.md` and `../DEV.md`.
