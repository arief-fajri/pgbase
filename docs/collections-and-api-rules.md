...
Collections map to one PostgreSQL table each; schema sync emits DDL per collection (`core/collection_record_table_sync.go`). System collections include `_superusers`, `_migrations`, `_logs` (aux DB), `_audits` + `_audit_reads` (month-range partitioned), `_externalAuths`, `_mfas`, `_otps`, `_authOrigins`.

Rules (`list/view/create/update/delete`) are the authorization layer. Realtime delivery re-checks them per message (view-rule for single-record subscriptions, list-rule for `*`).

## 2. Use the in-dashboard API preview (don't copy this doc)

The dashboard ships a live, collection-aware reference at `ui/src/apiPreview/` (19 files): `apiPreviewModal.js` tabs per operation with JS / Dart / curl snippets using the collection's actual fields and rules.
...
Expand is batched server-side (N+1 removed in v0.5.0); file fields serve via `apis/file.go:47` (Gzip opt-out) with superuser file-token flow.
