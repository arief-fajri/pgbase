...
> Single source of truth · Status: living document · Baseline: v0.5.2 (`923e860`) · Last review: 2026-09-08 (revised after independent code audit)
> All prior roadmap documents (roadmap-v2, v3, v4, v5, consolidation) are superseded by this file.
> **Published mirror:** the git-tracked copy lives at `docs/roadmap.md` (adapted: DocMeta frontmatter, docs-relative links). Keep both in sync; this file remains the working draft.
> **Progress note (2026-09-11, Sprint 0a COMPLETE):** all Sprint 0a items done on branch `phase-0` — `.github/SECURITY.md` forked; `FORK_STRATEGY.md` drift decision at repo root; CI now has blocking `-race` + golangci-lint jobs (162 pre-existing lint issues fixed: the v0.5.1 rebrand broke import order in ~154 files) + weekly `security-scan` (govulncheck + npm audit); lychee SHA-pinned with `failIfEmpty: true`; one-command provision shipped both ways (`deploy/quickstart.sh` curl|sh compose stack with pgvector-ready PG16 + `install.sh` checksum-verified binary; e2e verified); agent quick start (`docs/agents.md` + root `AGENTS.md`); positioning doc (`docs/comparison.md`); revised roadmap published to `docs/roadmap.md`. Remaining Sprint 0a loose ends: publish the ghcr.io image (needs the next `v*` tag push — the docker job ships with the branch), make the ghcr package public once, and publish the GitHub draft release after merge. Sprint 0b (pgvector, MCP, CLI contract) is next.
> Meta (resolved 2026-09-11): the roadmap is now tracked in git via the published mirror at `docs/roadmap.md`; the dangling-references issue in the docs pages was already resolved earlier (`docs/roadmap.md` restored via commit `17ab455`). `.feature-plan/` stays local-only for working drafts (this file, `checklist.md`, framework notes).
## 0. Product thesis
### Problem
...
| **Production deployment** (docker-compose.prod.yml with Caddy auto-HTTPS, monitoring stack under deploy/, production runbook) | `docker-compose.prod.yml`; `deploy/`; `docs/production.md` runbook |
...
See `docs/contributing.md`, `docs/developing.md`, and `docs/production.md` for implementation and operational guidance.
---
