...
// If you want to initialize the application before calling [PGBase.Start],
// then you'll have to manually call [PGBase.Bootstrap].
func NewWithConfig(config Config) *PGBase {
...
// so we can have all PG-BASE flags ready for use on initialization.
func (pb *PGBase) eagerParseFlags(config *Config) error {
	pb.RootCmd.PersistentFlags().StringVar(
		&pb.dataDirFlag,
		"dir",
		config.DefaultDataDir,
		"the PG-BASE data directory",
	)
...
func (pb *PGBase) skipBootstrap() bool {
	flags := []string{
		"-h",
		"--help",
		"-v",
