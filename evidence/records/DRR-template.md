# DRR-<issue#> — <short decision summary>

<!--
Decision Request Record for Level B decisions (docs/contributor/methodology/guardrails.md §8, G-AI-02/03).
File lives at evidence/records/DRR-<issue#>.md; the GitHub issue (template
.github/ISSUE_TEMPLATE/decision_request.yml) is the surface where a human confirms.

Level B: no auto-approval. Blocked until explicit maintainer confirmation.
Silence is not approval.
-->

## Decision

- **Authority level:** B
- **Guard rails / invariants implicated:** G-# / I-# (or "none, because <reason>")
- **Requester:** <agent / person>
- **Date opened:** YYYY-MM-DD
- **Status:** proposed / waiting / applied / rejected / withdrawn

## Context

<What is being decided, why now, constraints.>

## Options considered

| Option | Trade-offs |
|---|---|
| 1. | |
| 2. | |

## Recommendation

<Option + reasoning.>

## Risks / reversibility

<Blast radius, reversibility, rollback, and the evidence that will prove success.>

## Timeline & objection record

- Opened: YYYY-MM-DD HH:MM
- Objections: <none / details / dates>
- Confirmed: <maintainer / date>
- Applied: YYYY-MM-DD HH:MM (only after confirmation)

## Outcome

<What actually happened, evidence reference, and what was learned (link learnings.md).>